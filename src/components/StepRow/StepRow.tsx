/**
 * @module StepRow
 * Editor for a single Gherkin step. Renders an action-type selector, keyword
 * selector, and a type-specific parameter form (ActionParams). Also manages
 * inline and tabular data tables.
 *
 * The component is pure (controlled) — all state lives in the parent scenario.
 * Sub-components (EditorFields, RecordPicker, ValueInput, ActionParams) are
 * co-located here because they are tightly coupled to the step data model and
...
interface StepRowProps {
  ...
  step: Step;
  /** Called on every field change with the fully updated step. */
  onChange: (updated: Step) => void;
  /** Called when the user clicks the remove (×) button. */
  onRemove: () => void;
  /** Called when the user clicks the duplicate button. */
  onDuplicate: () => void;
  /** All available database/infosystem tables for field auto-complete. */
  tables: TableDef[];
  /**
   * The `tableRef` of the database currently open in the editor
   * (inferred by walking backwards from this step — see `currentTableRefAt`).
   * Used to filter field suggestions to the active table only.
   */
...
/**
 * Renders the full step editor: action-type dropdown, keyword dropdown,
 * free-text input (for `freetext` type), type-specific ActionParams, and
 * inline/tabular data table editors.
 *
 * Switching action types resets the action to sensible defaults and initialises
...
/** Props for the internal {@link ActionParams} switch component. */
...
/**
 * Shared sub-form for actions that open an editor (editorOeffnen variants).
 * Keeps `editorName` and `tableRef` in sync: selecting from the combobox
 * auto-fills the tableRef and vice versa, using the localized table name.
 */
...
function ActionParams({ action, onChange, tables, currentFields, createdRecords, onToggleMultiField, hasDataTable }: ActionParamsProps) {
  ...
  switch (action.type) {
    ...
    case 'feldPruefen':
      // If a data table is present, the single field inputs are not needed
