 ...
 * Editor for a single Gherkin scenario: name, optional comment, and a
 * drag-and-drop-reorderable list of steps.
 *
 * Handles both internal step reordering and drops from the external StepToolbox
 * panel. Toolbox drops are distinguished by the custom MIME type
 * `application/x-toolbox` so they can carry template or action-type payloads.
 */
...
/** Props for {@link ScenarioBuilder}. */
interface ScenarioBuilderProps {
  /** The scenario being edited. */
  scenario: Scenario;
  /** Called whenever any field of the scenario changes. */
  onChange: (updated: Scenario) => void;
  /** Called when the user removes this scenario. */
  onRemove: () => void;
  /** Available database/infosystem tables for field auto-complete. */
  tables: TableDef[];
  /** Records created by earlier `editorOeffnen NEW` steps in the feature — used for search-word picker. */
  createdRecords: CreatedRecord[];
  /** All search words already used in this feature — prevents duplicates in auto-generation. */
  existingSearchWords: string[];
}

/**
 * Walk backwards from `index` to find the `tableRef` (or `infosystemRef`) set
 * by the most recent "open editor/infosystem" step. This is used to provide
 * context-aware field suggestions in subsequent `feldSetzen` / `feldPruefen` steps.
 */
 ...
 *
 * Auto-fill: when an `editorOeffnen NEW` step receives an editor name, the
 * immediately following `feldSetzen such` step is automatically populated with
 * a generated search word (if it hasn't been manually edited yet).
 */
