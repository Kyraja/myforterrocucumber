 ...
 * Generic editable grid (rows × columns) used to author Gherkin Examples and
 * data tables inline. Supports adding/removing rows and optionally columns,
 * optional static column headers, and field-name autocomplete via a <datalist>
 * applied to either the first column or first row depending on
 * `fieldSuggestionsMode`.
 *
 * Key props: `table` – 2-D string array; `onChange` – called with the updated
 * table; `fields` – optional field suggestions for autocomplete.
 */
  ...
  return (
    <div className={styles.wrapper}>
      ...
      {fields && fields.length > 0 && (
        <datalist id={listId}>
          {fields.map((f, i) => (
            <option key={`${f.name}-${i}`} value={f.name}>{fieldDesc(f, lang)}</option>
          ))}
        </datalist>
      )}
