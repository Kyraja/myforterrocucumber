/**
 * @module FlowDiagram
 * SVG-based BPMN-style flow diagram that visualises one or more Gherkin scenarios.
 *
 * Key responsibilities:
 * - Renders each scenario as a vertical flow: start event → task boxes → end event.
 * - Color-codes task boxes by Gherkin phase: Given (setup/blue), When (action/orange), Then (check/green).
 * - Computes SVG layout dynamically based on step count and whether steps carry detail lines.
 * - Supports optional onStepClick callback to navigate to a specific step in the editor.
 * @prop {Scenario[]} scenarios - The scenarios to visualise; empty scenarios are omitted.
 * @prop {(stepId: string) => void} [onStepClick] - Optional callback fired when a task box is clicked.
 */
