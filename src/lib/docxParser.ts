 ...
 * @module docxParser
 *
 * DOCX document parser for importing requirement documents into the Gherkin generator.
 *
 * Uses mammoth to convert .docx → HTML, then splits the HTML by headings and
 * passes each chapter through {@link parseConsultantTemplate}.
 *
 * Three import modes are supported depending on the active {@link ParseProfile}:
 *
 * 1. **Technical section mode** (`splitting.technicalSectionKeywords` non-empty):
 *    Each chapter is split around a sub-heading like "Technische Umsetzung".
 *    Content before it becomes the Feature description; content after it is
 *    parsed for scenarios. Chapters without the keyword are skipped (no
 *    customization = no tests), keeping the skipped chapter list informative.
 *
 * 2. **Content end marker mode** (`splitting.contentEndKeywords` non-empty):
 *    The content end marker (e.g. "Auswirkungen der Customization/Extension")
 *    acts as a stop signal — everything before it is used as requirement text.
 *    Chapters without the marker are skipped.
 *
 * 3. **Legacy mode** (no splitting keywords): The entire chapter body is
 *    passed to the consultant template parser verbatim.
 *
 * Additionally generates a full document TOC and provides
 * {@link downloadDocxTemplate} to produce a ready-to-fill .docx template.
 *
 * Entry points: {@link parseDocx}, {@link downloadDocxTemplate}
 */
...
export async function parseDocx(file: File, profile?: ParseProfile): Promise<DocxImportResult> {
  ...
  // Compute chapter numbers for each heading (e.g. "1", "1.1", "1.2", "2")
  const chCounters: number[] = [];
  const headingKeys = allHeadings.map(({ text, level }) => {
    while (chCounters.length < level) chCounters.push(0);
    chCounters[level - 1]++;
    chCounters.splice(level);
    const chNum = chCounters.join('.');
    // Unique key: "3.4.3Bezeichnung" (no spaces, lowercase)
    return `${chNum}${text}`.replace(/\s+/g, '').toLowerCase();
  });

  // Build unique keys for features and skipped chapters the same way
  // (features/skipped are in document order, so we consume heading keys sequentially)
  const featureKeys = new Set<string>();
  const skippedKeys = new Set<string>();
  let fIdx = 0;
  let sIdx = 0;
  for (let h = 0; h < allHeadings.length; h++) {
    const hText = allHeadings[h].text;
    if (fIdx < features.length && features[fIdx].sourceHeading === hText) {
      featureKeys.add(headingKeys[h]);
      fIdx++;
    } else if (sIdx < skippedChapters.length && skippedChapters[sIdx].sourceHeading === hText) {
      skippedKeys.add(headingKeys[h]);
      sIdx++;
    }
  }

  // Now assign kind by unique key — each key appears exactly once
  const toc: TocEntry[] = allHeadings.map(({ text, level }, h) => ({
    text,
    level,
    kind: featureKeys.has(headingKeys[h]) ? 'package' as const
      : skippedKeys.has(headingKeys[h]) ? 'skipped' as const
      : 'structure' as const,
  }));
...
function htmlToPlainText(html: string): string {
  ...
  // Clean up: collapse consecutive empty lines to a single blank line
...
export async function downloadDocxTemplate(profile?: ParseProfile, lang: 'de' | 'en' = 'de'): Promise<void> {
  ...
  const txt = lang === 'en' ? {
    ...
    apStructureTitle: 'Mandatory AP structure (for CI and traceability):',
    apStructure: [
      '1. Beschreibung',
      '2. Technische Anpassung',
      '3. Abnahmekriterien',
    ],
    idRulesTitle: 'Mandatory IDs:',
    idRules: [
      'AP-xxxx for each work package',
      'BR-xxx for business rules',
      'AC-xxx for acceptance criteria',
      'SC-xxx for scenario mapping',
    ],
    ...
    ch1Br1: 'BR-001: For customer type "Retail", the category field is mandatory.',
    ch1Br2: 'BR-002: Existing records remain unchanged unless explicitly edited.',
    ch1Ac1: 'AC-001',
    ch1Ac2: 'AC-002',
    ch1Sc1: 'SC-001',
    ch1Sc2: 'SC-002',
    ...
    ch2Br1: 'BR-001: Weight must be greater than 0.',
    ch2Ac1: 'AC-001',
    ch2Sc1: 'SC-001',
    ...
    apStructureTitle: 'Verbindlicher AP-Aufbau (fuer CI und Traceability):',
    apStructure: [
      '1. Beschreibung',
      '2. Technische Anpassung',
      '3. Abnahmekriterien',
    ],
    idRulesTitle: 'Verbindliche IDs:',
    idRules: [
      'AP-xxxx pro Arbeitspaket',
      'BR-xxx fuer Business Rules',
      'AC-xxx fuer Abnahmekriterien',
      'SC-xxx fuer Szenario-Mapping',
    ],
    ...
    ch1Br1: 'BR-001: Bei Kundentyp "Retail" ist das Kategoriefeld Pflicht.',
    ch1Br2: 'BR-002: Bestehende Datensaetze bleiben unveraendert, bis sie bearbeitet werden.',
    ch1Ac1: 'AC-001',
    ch1Ac2: 'AC-002',
    ch1Sc1: 'SC-001',
    ch1Sc2: 'SC-002',
    ...
    ch2Br1: 'BR-001: Gewicht muss groesser als 0 sein.',
    ch2Ac1: 'AC-001',
    ch2Sc1: 'SC-001',
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        ...
        emptyLine(),
        bold(txt.apStructureTitle),
        ...txt.apStructure.map(para),
        emptyLine(),
        bold(txt.idRulesTitle),
        ...txt.idRules.map(para),
        emptyLine(),
        ...
        emptyLine(),
        bold('3. Abnahmekriterien'),
        para(`${txt.ch1Ac1} -> ${txt.ch1Sc1}: ${txt.ch1s1Name}`),
        para(`${txt.ch1Ac2} -> ${txt.ch1Sc2}: ${txt.ch1s2Name}`),
        emptyLine(),
        ...
        emptyLine(),
        bold('3. Abnahmekriterien'),
        para(`${txt.ch2Ac1} -> ${txt.ch2Sc1}: ${txt.ch2s1Name}`),
        emptyLine(),
