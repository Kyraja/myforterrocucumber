 ...
 *
 * File System Access API wrapper for directory-based .feature file management.
 *
 * Provides a high-level interface over the browser's File System Access API
 * (Chrome/Edge) for reading, writing, creating, deleting, moving, and renaming
 * .feature files and folders within a consultant-chosen project directory.
 *
 * Directory handles are persisted in IndexedDB so the workspace survives page
 * reloads without requiring the consultant to re-select the folder each time.
 * Two separate handle slots exist: the main project root and an optional FOP
 * (Function/Output/Process) directory for generated artefacts.
 *
 * Because the File System Access API has no native move or rename primitive,
 * those operations are implemented as read → write → delete sequences.
 *
 * Note: This API is only available in Chromium-based browsers.
 * Use {@link isFileSystemAccessSupported} to guard calls at runtime.
 */
...
export async function saveFopDirectoryHandle(handle: FileSystemDirectoryHandle): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    tx.objectStore(IDB_FILE_HANDLES_STORE).put(handle, IDB_FOP_HANDLE_KEY);
    tx.oncomplete = () => { db.close(); resolve(); };
    tx.onerror = () => { db.close(); reject(tx.error); };
  });
}

/** Load a previously saved FOP directory handle from IndexedDB */
export async function loadFopDirectoryHandle(): Promise<FileSystemDirectoryHandle | null> {
  try {
    const db = await openDb();
    return new Promise((resolve) => {
      const tx = db.transaction(IDB_FILE_HANDLES_STORE, 'readonly');
      const req = tx.objectStore(IDB_FILE_HANDLES_STORE).get(IDB_FOP_HANDLE_KEY);
...
/** Clear the saved FOP directory handle */
export async function clearFopDirectoryHandle(): Promise<void> {
  try {
    const db = await openDb();
    const tx = db.transaction(IDB_FILE_HANDLES_STORE, 'readwrite');
...
/** Create a new .feature file with empty template including a GUID tag */
 ...
 *
 * The File System Access API has no native move primitive, so this is
 * implemented as: read content → write to target → delete source.
 *
 * @param sourceParentHandle - Directory handle containing the source file
 * @param targetParentHandle - Destination directory handle
 * @param fileName - Name of the file to move (same in both directories)
 * @returns Handle to the newly created file in the target directory
 */
...
/**
 * Moves a directory from one parent to another by recursively copying all
 * contents to the target, then removing the original source tree.
 *
 * @param sourceParentHandle - Handle of the directory containing the source folder
 * @param targetParentHandle - Handle of the destination parent directory
 * @param dirName - Name of the directory to move
 */
 ...
 *
 * Because the File System Access API provides no rename primitive:
 * - Files are read, written under the new name, then the old entry is deleted.
 * - Directories are recursively copied to a new handle, then the old tree is removed.
 *   `.feature` extension is enforced for file renames.
 *
 * @param parentHandle - Directory that contains the entry to rename
 * @param oldName - Current name of the file or folder
 * @param newName - Desired new name (`.feature` suffix added automatically for files)
...
export async function readDirectoryTree(
  ...
  // Sort: folders first, then files — numeric-aware so "2" < "10"
  const numericCompare = (a: string, b: string) => a.localeCompare(b, undefined, { numeric: true });
  folders.sort((a, b) => numericCompare(a.displayName, b.displayName));
  files.sort((a, b) => numericCompare(a.displayName, b.displayName));
