...
describe('buildTableIdentificationMessages', () => {
  it('returns system + user message', () => {
    const msgs = buildTableIdentificationMessages('Anforderung');
  ...
  it('includes all database names', () => {
    // Tables are no longer passed to buildTableIdentificationMessages
    const msgs = buildTableIdentificationMessages('Anforderung');
    expect(msgs[1].content).toContain('Anforderung');
  });
  ...
  it('includes the requirements text', () => {
    const msgs = buildTableIdentificationMessages('Mein Anforderungstext hier');
    expect(msgs[1].content).toContain('Mein Anforderungstext hier');
  });

  it('instructs JSON-only response in system prompt', () => {
    const msgs = buildTableIdentificationMessages('Text');
...
describe('buildMessagesWithFields', () => {
  ...
  it('truncates fields beyond limit', () => {
    ...
    const msgs = buildMessagesWithFields('Text', tables);
    // Level 0 limits to maxStdFields:30 — field0 through field29 are shown
    expect(msgs[1].content).toContain('field0');
    expect(msgs[1].content).toContain('field29');
    expect(msgs[1].content).not.toContain('field30');
  });
  ...
  it('includes instruction to use only listed field names', () => {
    ...
    const msgs = buildMessagesWithFields('Text', tables);
    expect(msgs[1].content).toContain('AUSSCHLIESSLICH Feldnamen aus der obigen Liste');
  });
