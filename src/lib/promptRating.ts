...
/**
 * Score a requirement text against the defined rating criteria and collect
 * actionable suggestions for criteria that were not met.
 *
 * Each criterion contributes a fixed point value. The final `score` is
 * normalized to 0–100 relative to the total possible points across all
 * criteria, so adding new criteria never breaks existing scores.
 *
 * @param text - The raw requirement text (Anforderungstext) to evaluate.
 * @param hasDatabase - Whether the user has uploaded a variable/field table (CSV). Worth 15 pts.
 * @param lang - Language for suggestion and example texts (`'de'` or `'en'`).
 * @returns A {@link PromptRating} with a 0–100 score and a list of unmet suggestions.
 */
...
/**
 * Map a 0–100 prompt rating score to a CSS color value for visual feedback.
 *
 * Thresholds: ≥80 green (success), ≥60 amber (warning), ≥40 orange, <40 red (danger).
 * Uses CSS custom properties where available so the host theme can override them.
 *
 * @param score - A numeric score in the range 0–100.
 * @returns A CSS color string (custom property reference or hex fallback).
 */
