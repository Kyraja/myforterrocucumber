/**
 * @module consultantTemplate
 *
 * Parses consultant requirement documents written in a structured keyword format
 * into FeatureInput objects ready for Gherkin generation.
 *
 * The format uses German keyword prefixes (e.g. "Feature:", "Szenario:", "Aktion:")
 * that are defined by a ParseProfile. Both exact and fuzzy (Levenshtein) keyword
 * matching are supported to handle typos typical in on-site notes.
 *
 * Entry point: {@link parseConsultantTemplate}
 * Action syntax parser: {@link parseActionFromText}
 */
...
/**
 * Parses a step text string into a structured {@link StepAction}.
 *
 * Recognises the full abas action vocabulary (editor operations, field
 * get/set/check, table rows, buttons, subeditors, infosystems, dialogs,
 * exceptions) plus natural-language variants commonly used by consultants.
 * Custom action patterns from the active profile are tried last.
 * Falls back to `{ type: 'freetext' }` when nothing matches.
 *
 * @param text - The raw step text after the Gherkin keyword, e.g. `Feld setzen: such = "TEST"`
 * @param customActions - Optional custom regex patterns defined in the parse profile
 * @returns A discriminated union describing the parsed action
 */
...
/**
 * Parses a multi-scenario consultant document into a {@link FeatureInput}.
 *
 * The parser processes the document line-by-line, matching keywords against
 * the provided profile (defaulting to {@link DEFAULT_PARSE_PROFILE}). It
 * handles feature-level metadata (name, description, tags, database, test
 * user) as well as scenario blocks with their steps and comments.
 *
 * Consecutive steps of the same category (e.g. two "Aktion:" lines) are
 * automatically joined with the "And" keyword to produce valid Gherkin.
 *
 * @param text - Raw document text (UTF-8, any line endings)
 * @param profile - Optional custom parse profile; defaults to the standard German profile
 * @returns A fully populated FeatureInput ready to be rendered by generateGherkin()
 */
...
/**
 * Triggers a browser download of the plain-text format specification.
 * Useful for distributing the input format to consultants who fill in
 * documents manually without access to the tool.
 */
