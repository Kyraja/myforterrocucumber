/**
 * @module exampleWorkPackages
 *
 * Built-in example work package data and Excel export.
 *
 * Provides a set of realistic abas ERP customization work packages covering
 * common scenarios (new fields, validations, infosystems, subeditors, etc.)
 * that consultants can use as a reference or starting point.
 *
 * {@link downloadExampleXlsx} generates a properly formatted .xlsx file
 * that matches the column structure expected by {@link parseWorkPackageXlsx},
 * so consultants can use it as a template for their own work package lists.
 */
...
/**
 * Generates and triggers a browser download of an example work package Excel file.
 *
 * The generated file uses the column names recognized by {@link parseWorkPackageXlsx}
 * and is pre-populated with 12 representative abas ERP customization scenarios
 * spanning sales, purchasing, manufacturing, service, and controlling.
 */
