/**
 * Gherkin parser — converts raw `.feature` text back into the app's internal
 * {@link FeatureInput} data model.
 *
 * The parser handles two main responsibilities:
 *  1. **Structural parsing** — extracts Feature name, description, tags,
 *     Background (skipped), Scenarios, Steps, and data tables from plain text.
 *  2. **Action recognition** — maps each step text to the most specific known
 *     abas ERP step action via {@link parseStepAction}. Unrecognized steps fall
 *     back to `freetext` so they are preserved without loss.
 *
 * Post-parse transformations applied before returning:
 *  - Auto-search-word detection: marks `feldSetzen "such"` steps that follow a
 *    `NEW` editor open so the UI can offer to regenerate them.
 *  - Consecutive field-set merging: collapses adjacent `feldSetzen` steps with
 *    the same row into a single multi-field step with a data table, matching
 *    how the generator serializes them.
 */
...
function parseStepAction(rawText: string): StepAction {
  ...
  // Felder setzen (multi, mit Zeile): I set fields in row Z (data table follows)
  m = text.match(/^I set fields in row (\S+)$/);
  if (m) {
    return { type: 'feldSetzen', fieldName: '', value: '', row: m[1], multi: true };
  }
...
/**
 * Options for {@link parseGherkin}.
 */
export interface ParseGherkinOptions {
  /** When true, search words from AI are kept as-is (not marked for auto-overwrite) */
  fromAI?: boolean;
}

/**
 * Parse raw Gherkin `.feature` text into a {@link FeatureInput} structure.
 *
 * Detects known abas step patterns and creates structured actions via
 * {@link parseStepAction}. Unrecognized steps remain as `freetext`.
 *
 * Pass `options.fromAI = true` when processing AI-generated output to preserve
 * AI-supplied search words instead of flagging them for auto-regeneration.
 *
 * @param text - Raw Gherkin text to parse.
 * @param options - Optional parsing behaviour flags.
 * @returns A fully populated {@link FeatureInput} ready for the editor.
 */
export function parseGherkin(text: string, options?: ParseGherkinOptions): FeatureInput {
  ...
  // Mark search words as auto-generated (for local auto-fill cleanup).
  // Skip when parsing AI output — the AI's search words should be preserved.
  if (!options?.fromAI) {
      ...
      if (
        ...
        if (group.length >= 2) {
          ...
          merged.push({
            ...
            text: row ? `I set fields in row ${row}` : 'I set fields',
