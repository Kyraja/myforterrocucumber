...
Scenario: editable_new_entry_fields_general
    ...
    Given I open an editor "BuchungC" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "NEW" for record ""
    And I set field "such" to "B5"
    ...
    Given I open an editor "BuchungD" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "NEW" for record ""
    And I set field "such" to "B6"
    And I set field "budat" to "02.02.2002"
    And I set field "ewbu" to "USD"
  	And I create a new row at the end of the table
    And I set field "konto" to "k 1" in row 1
    And I create a new row at the end of the table
    And I set field "konto" to "44000USD" in row 2
    And I set field "ewhbetr" to "100" in row 2
    And I set field "kstelle" to "100" in row 2
    And I respond with answer "Ja" to the dialog with id "7709"
    And I save the current editor
    
 Given I open an editor "Buchung1" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3"
...
Scenario: editable_update_w_entry_fields_only_fibu
 
 Given I open an editor "Buchung1" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3"
    Then field "ebeleg" is modifiable
    Then field "buartrdiff" is modifiable
    And I close the current editor
     
Scenario: editable_update_w_entry_fields_consolidation
...
Scenario: editable_update_w_entry_fields_currency    
 Given I open an editor "Buchung2" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3"
...
Scenario: editable_update_w_entry_fields_payment
    Given I open an editor "Buchung3" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3"
...
Scenario: editable_update_w_entry_fields_action
    Given I open an editor "Buchung4" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3"
...
Scenario: editable_update_w_entry_fields_head_tax
    Given I open an editor "Buchung5" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3"
    Then field "butyp" is modifiable
    ...
    Then field "zmrel" is modifiable
    Then field "ustidfa" is not modifiable

    And I close the current editor
        
Scenario: editable_update_w_entry_fields_set_of_books
    Given I open an editor "Buchung6" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3" 
...
Scenario: editable_update_w_entry_fields_row_general
    Given I open an editor "Buchung7" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B4" 
...
Scenario: editable_update_w_entry_fields_row_advance_payment
    Given I open an editor "Buchung8" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3" 
...
Scenario: editable_update_w_entry_fields_kv
    Given I open an editor "Buchung9" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3" 
    ...
    Then field "kvoffen" is not modifiable in row 2
    Then field "kv" is not modifiable in row 2
    Then field "kvrel" is not modifiable in row 2
    Then field "kvnum" is modifiable in row 2
    And I close the current editor

Scenario: editable_update_w_entry_fields_w2
    Given I open an editor "Buchung10" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B4" 
    ...
    Then field "kow2" is not modifiable in row 2
    Then field "w2kurs" is not modifiable in row 2
    Then field "w2ewkurs" is not modifiable in row 2
    Then field "w2einh" is not modifiable in row 2
    Then field "w2ekurs" is not modifiable in row 2
    Then field "w2betr" is not modifiable in row 2
    Then field "w2hbetr" is modifiable in row 2
    Then field "w2gjahr" is not modifiable in row 2
    And I close the current editor 

Scenario: editable_update_w_entry_fields_fibu_general
    Given I open an editor "Buchung11" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3" 
...
Scenario: editable_update_w_entry_fields_row_tax
    Given I open an editor "Buchung12" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3" 
    Then field "strgl" is not modifiable in row 1
    Then field "zstrgl" is not modifiable in row 1    
    Then field "steuer" is modifiable in row 1
    Then field "ev" is not modifiable in row 1  
    Then field "laart" is modifiable in row 1
    Then field "stbasis" is not modifiable in row 1    
    Then field "zmart" is modifiable in row 1
    Then field "ustva" is modifiable in row 1
#
    Then field "strgl" is modifiable in row 2
    Then field "zstrgl" is not modifiable in row 2    
    Then field "steuer" is modifiable in row 2
    Then field "ev" is not modifiable in row 2  
    Then field "laart" is modifiable in row 2
    Then field "stbasis" is not modifiable in row 2    
    Then field "zmart" is modifiable in row 2
    Then field "ustva" is modifiable in row 2
#
    ...
    Then field "ustart" is modifiable in row 1    
...
Scenario: editable_update_w_entry_fields_row_currency
    Given I open an editor "Buchung12" from table "(RecurringEntry):(FinancialEntryTemplate)" with command "UPDATE" for record "B3" 
