@persistant
Feature: Copying statistical entries and statistical entry templates
Background:
Given I set the fake date to "2.2.2002"

Scenario: create_statistical_entry
...
Scenario: copy_statistical_entry_to_template
    Given I open an editor "Buchung2" from table "(RecurringEntry):(StatisticalEntryTemplate)" with command "COPY" for record "CP1BU"
    And I set field "such" to "CP1TMP" 
    And I respond with answer "Ja" to the dialog with id "7709"
    And I save the current editor
    And I close the current editor  

Scenario: copy_template_to_statistical_entry
    Given I open an editor "Buchung3" from table "(Entry):(StatisticalEntry)" with command "COPY" for record "110 CP1TMP"
    And I respond with answer "Ja" to the dialog with id "583"
    And I save the current editor
    And I close the current editor  
    
Scenario: copy_statistical_entry_without_bukonto
    Given I open an editor "Buchung3" from table "(Entry):(StatisticalEntry)" with command "COPY" for record "SBU03"
