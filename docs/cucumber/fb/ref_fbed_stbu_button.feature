...
Scenario: delete_row
    ...
    Given I open an editor "Buchung2" from table "(Entry):(StatisticalEntry)" with command "UPDATE" for record "EDI1"
    Then field "gegen1" has value "99800" in row 1
    And I delete row at position 2
    Then field "gegen1" has value "" in row 1
    And I create a new row at the end of the table
    And I set field "konto" to "99800" in row 2
    And I set field "hbetrag" to "100" in row 2
    And I set field "kstelle" to "100" in row 2
