...
Scenario: validate_field_bubudat_wartung    
    ...
    Given I open an editor "Buchung3" from table "(Entry):(StatisticalEntry)" with command "UPDATE" for record "SBU01"
...
Scenario: validate_field_bumonat_sy    
    ...
    And I close the current editor
    Given I open an editor "Buchung5" from table "(Entry):(StatisticalEntry)" with command "UPDATE" for record "SBU01"
    And setting field "monat" to "-1" throws the exception "131"
    And setting field "monat" to "16" throws the exception "1361"
    And setting field "monat" to "1" throws the exception "1361"
    And I close the current editor
    Given I open an editor "Buchung6" from table "(Entry):(StatisticalEntry)" with command "COPY" for record "SBU01"
...
Scenario: validate_field_bukprojekt 
    Given I'm logged in with password "sy"
    Given I open an editor "Buchung10" from table "(Entry):(StatisticalEntry)" with command "NEW" for record ""  
    And setting field "kprojekt" to "100000" throws the exception "1361"
    And I close the current editor  
    
Scenario: validate_field_bukonto
	Given I open an editor "Buchung11" from table "(Entry):(StatisticalEntry)" with command "NEW" for record ""
...
Scenario: validate_field_bukstelle
	Given I open an editor "Buchung12" from table "(Entry):(StatisticalEntry)" with command "NEW" for record ""
    And I set field "such" to "SB5"
	And I create a new row at the end of the table
	...
	# muss den passenden dyn. kostenverteiler finden (auch in std/test/cucumber/fb/ref_fbed_dbusbv_validate_field.feature)
	...
	Given I open an editor "Buchung14" from table "(Entry):(StatisticalEntry)" with command "UPDATE" for record "SB5"
